...
instance ToAbstract C.Expr where
  ...
  -- Literals
      C.Lit r l -> do
        ...
        ensureInScope v@(Just (I.Def q _)) = do
          inverseScopeLookupName' AmbiguousNothing q <$> getScope >>= \case
            C.QName{} : _ -> return v -- NOTE: inverseScopeLookupName puts unqualified names first
            _             -> return Nothing
